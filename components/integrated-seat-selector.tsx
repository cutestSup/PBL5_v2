...
import { Check, Info, Bus } from "lucide-react"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Separator } from "@/components/ui/separator"
import { Badge } from "@/components/ui/badge"

interface IntegratedSeatSelectorProps {
  busType: string
...
export function IntegratedSeatSelector({
  busType,
  availableSeats,
  onSelectSeats,
  selectedSeats,
  ...
  const [seatLayout, setSeatLayout] = useState<{
    ...
    seatMap: { [key: string]: { type: string; position: string; floor?: string } }
  }>({
    rows: 0,
    cols: 0,
    totalSeats: 0,
  ...
  useEffect(() => {
    const layout = {
      ...
      seatMap: {} as { [key: string]: { type: string; position: string; floor?: string } },
    }
      ...
      case "Giường nằm":
        ...
        for (let row = 1; row <= layout.rows; row++) {
          for (let col = 0; col < layout.cols; col++) {
            ...
            // Determine seat floor (upper/lower)
            const floor = row <= 5 ? "Tầng dưới" : "Tầng trên"

            // Determine seat position
            let position = "Giữa"
            if (col === 0) position = "Cửa sổ"
            if (col === 2) position = "Lối đi"
            ...
            layout.seatMap[seat] = { type, position, floor }
          }
        }
        break
      ...
      default:
        ...
        for (let row = 1; row <= layout.rows; row++) {
          for (let col = 0; col < layout.cols; col++) {
            ...
            // Determine seat position
            let position = "Giữa"
            if (col === 0 || col === 3) position = "Cửa sổ"
            if (col === 1 || col === 2) position = "Lối đi"

            // Determine seat type
            const type = "Ghế thường"
            ...
            <Tooltip>
              ...
              <TooltipContent side="top" className="text-xs">
                <div className="space-y-1">
                  ...
                  {seatInfo.floor && <p>{seatInfo.floor}</p>}
